...
def run(workspace_path: str, until_complete: bool = False, approval_mode: bool = False) -> None:
    ...
    try:
        ...
        if not _pl.Path(plan_path).exists():
            ...
            _plan_source = plan_data.get("source", "deepseek")
            if _plan_source == "local_fallback":
                activity.log(
                    f"Mission plan: {task_count} tasks (LOCAL FALLBACK — DeepSeek unavailable)"
                )
            else:
                activity.log(
                    f"Mission plan generated by DeepSeek: {task_count} tasks in .romyq/plan.json"
                )
    except Exception:
        pass
    ...
    try:
        ...
        if not _pl2.Path(lc_path).exists():
            ...
            _lc_source = _lc_data.get("source", "deepseek")
            if _lc_source == "local_fallback":
                activity.log(
                    f"Lifecycle: {_phase_count} phases (LOCAL FALLBACK — DeepSeek unavailable)"
                )
            else:
                activity.log(
                    f"Lifecycle generated by DeepSeek: {_phase_count} phases in .romyq/lifecycle.json"
                )
        else:
    ...
    def _heartbeat_cb(elapsed: int) -> None:
        ...
        if _live_status:
            sys.stdout.write(f"\rClaude | Executing | {e_fmt} elapsed | {r_fmt} remaining   ")
            sys.stdout.flush()
        else:
